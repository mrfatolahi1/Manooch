...
func (a *Adapter) FetchMetadata(ctx context.Context, marketType manoochv1.MarketType) ([]*manoochv1.InstrumentMeta, error) {
	...
	for _, m := range info.Symbols {
		...
		out = append(out, &manoochv1.InstrumentMeta{Env: &manoochv1.Envelope{Venue: Venue, Instrument: ref.Proto(orderSymbol(ref)), Channel: manoochv1.Channel_CHANNEL_METADATA, ExchangeTimeNs: exchangeNs, RecvTimeNs: received, ExchangeTimeIsSendTime: info.ServerTime > 0, Source: manoochv1.Source_SOURCE_REST, Status: manoochv1.Status_STATUS_HEALTHY}, TickSize: int64(tick), LotSize: int64(lot), MinSize: int64(lot), ContractMultiplier: int64(price.SizeScale), Active: m.Status == "TRADING", LastRefreshNs: received})
	}
	if len(out) == 0 {
