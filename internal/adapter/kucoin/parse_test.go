...
func TestOneTopicTwoSubjects(t *testing.T) {
	...
	messages, err := adapter.Parse([]byte(`{"topic":"/contract/instrument:XBTUSDTM","type":"message",`+
		`"subject":"mark.index.price","data":{"markPrice":90445.02,"indexPrice":90440.135,`+
		`"granularity":1000,"timestamp":1731899129000}}`), adaptertest.ReceivedNs)
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
	if len(messages) != 2 {
		t.Fatalf("mark.index.price produced %d messages, want 2", len(messages))
	}
	wantKeys := []string{
	...
	for i, want := range wantKeys {
		...
		if messages[i].TimeToLive != 3*time.Second {
			t.Errorf("message %d ttl = %v, want 3s", i, messages[i].TimeToLive)
		}
	}
	...
	for i, message := range messages {
		...
		if envelope.Instrument.VenueSymbol != "XBTUSDTM" {
			t.Errorf("message %d venue_symbol = %q", i, envelope.Instrument.VenueSymbol)
		}
	}
	...
	if got := price.Price(messages[0].Proto.(*manoochv1.MarkPrice).MarkPrice).String(); got != "90445.02" {
		t.Errorf("mark price = %s, want 90445.02", got)
	}
	if got := price.Price(messages[1].Proto.(*manoochv1.IndexPrice).IndexPrice).String(); got != "90440.135" {
	...
	funding, err := adapter.Parse([]byte(`{"topic":"/contract/instrument:XBTUSDTM","subject":"funding.rate",`+
		`"data":{"granularity":60000,"fundingRate":-0.002966,"timestamp":1551770400000}}`), adaptertest.ReceivedNs)
	if err != nil {
	...
	if funding[0].TimeToLive != 180*time.Second {
		t.Errorf("funding ttl = %v, want 180s", funding[0].TimeToLive)
	}
	fundingPayload := funding[0].Proto.(*manoochv1.Funding)
	if got := price.Rate(fundingPayload.FundingRate).String(); got != "-0.002966" {
	...
	if fundingPayload.NextFundingTimeNs != 0 {
		t.Errorf("next_funding_time_ns = %d; KuCoin does not supply it on this stream", fundingPayload.NextFundingTimeNs)
	}
	...
	if fundingPayload.IntervalSeconds != 0 {
		t.Errorf("interval_seconds = %d; the payload's granularity is a push cadence", fundingPayload.IntervalSeconds)
	}
}
...
func TestUnquotedNumbersKeepTheirDigits(t *testing.T) {
	...
	messages, err := adapter.Parse([]byte(`{"topic":"/contract/instrument:ETHUSDTM","type":"message",`+
		`"subject":"mark.index.price","data":{"markPrice":1234567.89012345678,`+
		`"indexPrice":0.00000000001,"granularity":1000,"timestamp":1731899129000}}`), adaptertest.ReceivedNs)
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
	if got := messages[0].Proto.(*manoochv1.MarkPrice).MarkPrice; got != 123456789012345678 {
		t.Errorf("mark price = %d, want 123456789012345678 exactly", got)
	}
	if got := messages[1].Proto.(*manoochv1.IndexPrice).IndexPrice; got != 1 {
...
func TestWrongTimestampUnitIsRejected(t *testing.T) {
	...
	for name, timestamp := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			messages, err := adapter.Parse([]byte(`{"topic":"/contract/instrument:XBTUSDTM","type":"message",`+
				`"subject":"mark.index.price","data":{"markPrice":90445.02,"indexPrice":90440.135,`+
				`"granularity":1000,"timestamp":`+timestamp+`}}`), adaptertest.ReceivedNs)
			if len(messages) != 0 {
				t.Errorf("Parse produced %d messages for a %s timestamp", len(messages), name)
			}
...
func TestErrorFrameIsTyped(t *testing.T) {
	adapter := newAdapter(t)
	messages, err := adapter.Parse([]byte(`{"id":"sub-0","type":"error","code":404,"data":"topic not supported"}`), adaptertest.ReceivedNs)
	if len(messages) != 0 {
		t.Errorf("Parse produced %d messages alongside a venue error", len(messages))
	}
	var parseError *core.ParseError
	if !errors.As(err, &parseError) {
		t.Fatalf("Parse error = %v (%T), want *core.ParseError", err, err)
...
func TestZeroFundingRateIsPublished(t *testing.T) {
	adapter := newAdapter(t)
	messages, err := adapter.Parse([]byte(`{"topic":"/contract/instrument:XBTUSDTM","subject":"funding.rate",`+
		`"data":{"granularity":60000,"fundingRate":0,"timestamp":1551770400000}}`), adaptertest.ReceivedNs)
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
	if len(messages) != 1 {
	...
	if got := messages[0].Proto.(*manoochv1.Funding).FundingRate; got != 0 {
		t.Errorf("funding rate = %d, want 0", got)
	}
	if messages[0].TimeToLive != 180*time.Second {
		t.Errorf("ttl = %v, want 180s", messages[0].TimeToLive)
	}
}
...
func TestUnknownSymbolAndSubjectAreClassified(t *testing.T) {
	...
	for name, frame := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			messages, err := adapter.Parse([]byte(frame), adaptertest.ReceivedNs)
			if len(messages) != 0 {
				t.Errorf("Parse produced %d messages", len(messages))
			}
			var parseError *core.ParseError
			if !errors.As(err, &parseError) {
				t.Fatalf("Parse error = %v (%T), want *core.ParseError", err, err)
...
func TestMalformedFrameDoesNotPanic(t *testing.T) {
	...
	for _, frame := range []string{
		...
		messages, _ := adapter.Parse([]byte(frame), adaptertest.ReceivedNs)
		if len(messages) != 0 {
			t.Errorf("Parse(%q) produced %d messages", frame, len(messages))
		}
	}
}
