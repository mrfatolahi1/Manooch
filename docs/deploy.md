...
| `.gitignore` | Ignores `bin/`, `dist/`, `.local/` (generated by `make run`) and `testdata/raw/` (written by `manooch-tap --raw`) |

### Allowed dependencies

The complete list of direct dependencies. Everything else in `go.mod` is transitive.
...
No application framework, no ORM, no CLI or config library: `net/http.ServeMux` and `flag` cover what is needed, and a framework would add a configuration surface and an upgrade treadmill for no capability the service uses.
...
- **Add a dependency only if it is on the list above.** The list is the policy; a new direct dependency is a decision to make deliberately, not a side effect of reaching for a convenience.
