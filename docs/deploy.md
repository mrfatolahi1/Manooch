Covers: M0 · `deploy/`, `Makefile`, `go.mod`, `.dockerignore`, `.gitignore`
...
| `client-output-buffer-limit pubsub` | `256mb 64mb 60` | The default is crossed by a book firehose, and Redis drops an over-budget subscriber with no notice to either side. Raising it makes that rarer, not impossible — consumers still watch `publish_seq` |
